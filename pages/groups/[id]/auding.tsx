        ...
        <div className={`${data && goodAnswers.includes(data[i]?.id) && BG_SUCCESS} w-full min-h-[100px] sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 mx-auto flex flex-col rounded-lg border-2 shadow-md p-4`}>
            ...
            <>
                ...
                <div className='flex justify-center pb-2'>
                    <TextInput autoFocus ref={input} className='w-full text-center' disabled={goodAnswers.includes(data[i].id)} value={goodAnswers.includes(data[i].id) ? data[i].eng : answer} onChange={(e)=>setAnswer(e.target.value)}/>
                </div>
