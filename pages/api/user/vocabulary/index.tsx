...
export default async function handler(req: NextApiRequest, res:NextApiResponse) {
    try{
        ...
        if(req.method === "GET"){
            const { english, russian, auding, spelling, speaking } = await prisma.user.findUnique({
                where: {
            ...
            english.forEach(word => { //(О)n
                result.push({...word, english: true, russian: false, spelling: false, auding: false, speaking: false})
            })
            russian.forEach(word => { //(О)1/2n^2
                ...
                if(i > -1){
                    ...
                    result.push({...word, english: false, russian: true, spelling: false, auding: false, speaking: false })
                }
            })
            auding.forEach(word => { //(О)1/2n^2
                ...
                if(i > -1){
                    ...
                    result.push({...word,  english: false, russian: false, spelling: false, auding: true, speaking: false })
                }
            })
            spelling.forEach(word => { //(О)1/2n^2
                ...
                if(i > -1){
                    ...
                    result.push({...word,  english: false, russian: false, spelling: true, auding: false, speaking: false })
                }
            })
            speaking.forEach(word => { //(О)1/2n^2
                const i = result.findIndex(el => el.id === word.id)
                if(i > -1){
                    result[i] = {...result[i], speaking: true}
                }else{
                    result.push({...word,  english: false, russian: false, spelling: false, auding: false, speaking: true })
                }
            })
            // (О)1/2n^2*4 + //(О)n = (О)2n^2 + (O)n
            //Загрузка при таком подходе минимальная 400-450мс
            //Средняя в диапазоне 400-1000мс
            //Бывают и до 1500мс 
            //Но при возврате просто data = { english, russian, auding, spelling, speaking } - минимальная загрузка около 380мс но средняя примерно такаяже, но после деплоя это может поменяться
        ...
        if(req.method === "PUT"){  
            const data = await prisma.user.update({
                ...
                data: {
                    ...
                    speaking: {
                        connect: method === SPEAKING ? { id: word_id } : undefined
                    },
                },
                select: {
                    ...
                    speaking: method === SPEAKING ? true : undefined,
                }
            })
        ...
        if(req.method === "DELETE"){
            const data = await prisma.user.update({
                ...
                data: {
                    ...
                    speaking: {
                        disconnect: method === SPEAKING ? { id: word_id } : undefined
                    },
                },
                select: {
                    english: method === 'ENGLISH' ? true : undefined,
